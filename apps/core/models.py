...
# Detect Postgres without importing DB connection at import-time
POSTGRES = 'postgres' in settings.DATABASES['default']['ENGINE'].lower()
...
if POSTGRES:
    ...
    AUDITLOG_INDEXES.append(GinIndex(fields=['details'], name='auditlog_details_gin'))
...
class AddressQuerySet(models.QuerySet):
    ...
    def city(self, name: str):
        return self.filter(city__iexact=(name or '').strip())

    def country(self, name: str):
        return self.filter(country__iexact=(name or '').strip())
...
class AuditLogQuerySet(models.QuerySet):
    ...
    def action(self, action: str):
        return self.filter(action__iexact=(action or '').strip())
